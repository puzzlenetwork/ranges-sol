...
import "./BaseRangePool.sol";
import "./RangePoolProtocolFees.sol";

/**
 * @dev Range Pool with immutable weights.
 */
contract RangePool is BaseRangePool, RangePoolProtocolFees {
    ...
    constructor(
        ...
        RangePoolProtocolFees(params.tokens.length, params.rateProviders)
    {
        ...
        _token8 = numTokens > 8 ? params.tokens[8] : IERC20(0);
        _token9 = numTokens > 9 ? params.tokens[9] : IERC20(0);
        ...
        _scalingFactor8 = numTokens > 8 ? _computeScalingFactor(params.tokens[8]) : 0;
        _scalingFactor9 = numTokens > 9 ? _computeScalingFactor(params.tokens[9]) : 0;
        ...
        _normalizedWeight8 = numTokens > 8 ? params.normalizedWeights[8] : 0;
        _normalizedWeight9 = numTokens > 9 ? params.normalizedWeights[9] : 0;
    }
    ...
    function _changeVirtualBalance(IERC20 token, uint256 delta, bool increase) internal virtual override {
        uint256 tokenIdx = _getTokenIndex(token);
        uint256 downScaledDelta = _downscaleDown(delta, _scalingFactor(token));
        _virtualBalances[tokenIdx] = increase ? _virtualBalances[tokenIdx].add(downScaledDelta) :
            _virtualBalances[tokenIdx].sub(downScaledDelta);
    }

    function _setVirtualBalances(uint256[] memory balances) internal virtual override {
        for (uint256 i = 0; i < _totalTokens; i++) {
            _virtualBalances[i] = balances[i];
        }
    }

    function _changeVirtualBalancesBy(uint256 ratioMin, bool increase) internal virtual override {
        for (uint256 i = 0; i < _totalTokens; i++) {
            uint256 value = _virtualBalances[i].mulUp(ratioMin).divDown(FixedPoint.ONE);
            _virtualBalances[i] = increase ? _virtualBalances[i].add(value) : _virtualBalances[i].sub(value);
        }
    }
    ...
    function _updatePostJoinExit(uint256 postJoinExitInvariant)
        internal
        virtual
        override(BaseRangePool, RangePoolProtocolFees)
    {
        RangePoolProtocolFees._updatePostJoinExit(postJoinExitInvariant);
    }
    ...
    function _isOwnerOnlyAction(bytes32 actionId)
        ...
        view
        virtual
        override(BasePool, RangePoolProtocolFees)
        returns (bool)
    {
